...
import { useMutation as rahUseMutation } from 'react-apollo-hooks';
...
const ApolloHooksComponent = () => {
  const [createItem, { error, data, loading }] = useMutation(mutation, { onError: noop });
  const onClick = () => createItem({ variables: { name: 'new item' } });
...
const HookedMutatorComponent = () => {
  ...
  const [createItem] = rahUseMutation(mutation);
  const onClick = () =>
...
describe('mutations', () => {
  ...
  describe('wrapped mutation component', () => {
    it('allows to mock mutation queries', () => {
      ...
      const wrapper = render(<ApolloHooksComponent />);
      expect(wrapper.html()).to.eql('<button>click me</button>');
